...
def map_clip_view(azimuth):

        if isinstance(azimuth, torch.Tensor):
            angle = azimuth.item()
        else:
            angle = azimuth  # 假设是一个浮点数
    ...
    def train_step(self, data, save_guidance_path:Path=None):
        ...
        # novel view loss
            ...
            if 'SD' in self.guidance:
                ...
                clip_view = map_clip_view(azimuth)
                ...
                # 打印azimuth的尺寸
                # print("azimuth的尺寸:", azimuth.shape, "azimuth的尺寸2", azimuth.shape[0])
                # print("azimuth的大小:", azimuth)
                ...
                if self.opt.perpneg:
                    ...
                    loss_, clip_loss = self.guidance['SD'].train_step(text_z,  azimuth[0], pred_rgb, as_latent=as_latent, guidance_scale=self.opt.guidance_scale, grad_scale=self.opt.lambda_guidance,
                                                                save_guidance_path=save_guidance_path, clip_view = clip_view)
                    loss = loss + loss_
        ...
        return pred_rgb, pred_depth, loss, clip_loss
    ...
    def train_one_epoch(self, loader, max_epochs):
        ...
        for data in loader:
            ...
            with torch.cuda.amp.autocast(enabled=self.fp16):
                ...
                pred_rgbs, pred_depths, loss, clip_loss = self.train_step(data, save_guidance_path=save_guidance_path)

                clip_losses.append(clip_loss.item())

                # 打印clip_loss的值
                # print(f"itr {self.global_step}CLIP loss: {clip_loss.item():.4f}")


            # ----- 判断 clip_loss 是否有效 -----
            if clip_loss.item() < 0.13:
                # 视为无效迭代，跳过反向传播和优化步骤
                if self.local_rank == 0:
                    # print(f"Skipped itr {self.global_step} due to low CLIP loss: {clip_loss.item():.4f}")
                    pbar.update(loader.batch_size)
        ...
        # clip_loss log
        max_loss = max(clip_losses)
        min_loss = min(clip_losses)
        avg_loss = sum(clip_losses) / len(clip_losses)

        self.log(f"==> [{time.strftime('%Y-%m-%d_%H-%M-%S')}] Finished Epoch {self.epoch}/{max_epochs}. "
           f"CPU={cpu_mem:.1f}GB, GPU={gpu_mem:.1f}GB, Max CLIP loss: {max_loss:.4f}, "
           f"Min CLIP loss: {min_loss:.4f}, Avg CLIP loss: {avg_loss:.4f}")
